                ...
                {messages.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-center space-y-8 animate-in fade-in duration-1000 relative z-10">
                        <div className="relative group">
                            <div className="absolute inset-0 bg-[#007ea7]/20 rounded-full blur-3xl opacity-50 group-hover:opacity-100 transition-opacity" />
                            <div className="w-24 h-24 bg-white rounded-[2.5rem] flex items-center justify-center text-[#007ea7] border-2 border-slate-50 shadow-3xl relative z-10 group-hover:rotate-12 transition-transform">
                                <Activity size={40} className="animate-pulse" strokeWidth={3} />
                            </div>
                        </div>
                        <div className="space-y-4">
                            <p className="text-[12px] font-black uppercase tracking-[6px] text-[#003249] italic">Mission Manifest: Clear</p>
                            <p className="text-[10px] font-bold text-slate-300 uppercase tracking-[4px] leading-relaxed italic max-w-[280px] mx-auto">System nodes have stabilized. Awaiting initial signal transmission from terminal.</p>
                        </div>
                    </div>
                ) : (
                                ...
                                <div className={`flex gap-8 max-w-[90%] md:max-w-[75%] ${isMe ? 'flex-row-reverse text-right' : 'text-left'}`}>
                                    ...
                                    <div className="flex-shrink-0 pt-2">
                                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-3xl border-2 transition-all duration-500 hover:rotate-12 relative overflow-hidden ${
                                            isMe ? 'bg-[#007ea7] border-[#007ea7] text-[#003249]' : 'bg-[#003249] border-[#003249] text-white'
                                        }`}>
                                    ...
                                    <div className={`space-y-4 ${isMe ? 'items-end' : 'items-start'} flex flex-col`}>
                                        <div className={`p-8 md:p-10 rounded-[3rem] shadow-3xl relative overflow-hidden group transition-all duration-500 ${isMe
                                                ? 'bg-[#003249] text-white rounded-tr-none border border-white/5'
                                                : 'bg-white border-2 border-slate-50 rounded-tl-none text-[#003249]'
                                            }`}>
                                            {/* Decorative Element */}
                                        ...
                                        <div className={`flex items-center gap-4 px-4 text-[9px] font-black uppercase tracking-[4px] opacity-30 italic ${isMe ? 'flex-row-reverse text-right' : ''}`}>
                                            <div className="flex items-center gap-2">
                                                <div className={`w-1.5 h-1.5 rounded-full ${isMe ? 'bg-[#007ea7]' : 'bg-[#003249]'}`} />
                                                <span className={isMe ? 'text-[#007ea7]' : 'text-[#003249]'}>{isMe ? 'FIELD_OPERATIVE_ALPHA' : 'CENTRAL_COMMAND_NODE'}</span>
                                            </div>
                                            <div className="w-px h-3 bg-slate-300" />
                                            <span className="text-slate-400 font-bold">{new Date(msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>
                                        </div>
                                    </div>
                                </div>
                            </motion.div>
            ...
            <form onSubmit={handleSend} className="p-8 md:p-12 bg-slate-50/50 backdrop-blur-xl border-t-2 border-slate-50 relative z-20">
                <div className="max-w-5xl mx-auto flex gap-6 relative">
                    <div className="flex-1 relative group/input">
                        <div className="absolute left-10 top-1/2 -translate-y-1/2 flex items-center gap-4 text-slate-200 group-focus-within/input:text-[#007ea7] transition-all">
                             <Terminal size={22} strokeWidth={3} />
                             <div className="w-0.5 h-6 bg-slate-100 group-focus-within/input:bg-[#007ea7]/20" />
                        </div>
                        <input
                            type="text"
                            ...
                            className="w-full pl-24 pr-10 h-20 bg-white border-2 border-slate-50 rounded-[2rem] focus:border-[#007ea7] outline-none font-black text-[12px] uppercase tracking-[4px] text-[#003249] shadow-inner transition-all focus:ring-8 focus:ring-[#007ea7]/5 italic"
                        />
                    </div>
                    <button
                        type="submit"
                        disabled={!newMessage.trim()}
                        className="w-24 h-20 bg-[#003249] text-[#007ea7] rounded-[2rem] shadow-3xl flex items-center justify-center transition-all hover:bg-[#007ea7] hover:text-white active:scale-95 disabled:opacity-30 disabled:grayscale group relative overflow-hidden"
                    >
                         <div className="absolute inset-0 bg-gradient-to-br from-white/10 to-transparent pointer-events-none" />
                    ...
                    <div className="absolute -top-12 left-10 flex items-center gap-6 opacity-40">
                         <div className="flex items-center gap-2 text-[8px] font-black uppercase tracking-[4px] text-[#003249] italic">
                ...
                <div className="mt-8 flex justify-center">
                    <div className="flex items-center gap-4 px-6 py-2 bg-white/50 border border-slate-100 rounded-full shadow-sm">
                        <p className="text-[8px] font-black uppercase tracking-[5px] text-slate-300 flex items-center gap-3 italic">
                            <Lock size={12} strokeWidth={3} className="text-[#007ea7]" /> End-to-End Quantum Encryption Protocol Active
                        </p>
                    </div>
                </div>
            </form>

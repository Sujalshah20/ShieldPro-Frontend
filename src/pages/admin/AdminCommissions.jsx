...
import { 
    ...
    ArrowUpRight, Download, Satellite, Cpu,
    Target, Activity, Lock, Layers, CreditCard,
    ArrowDownRight, Box
} from "lucide-react";
...
const AdminCommissions = () => {
    ...
    const statusMutation = useMutation({
        ...
        onSuccess: (data, variables) => {
            ...
            toast({ title: "YIELD_SETTLEMENT_SYNCED", description: `Commission node marked as ${variables.status.toUpperCase()} in the global ledger.` });
        },
        onError: (err) => {
            toast({
                title: "SETTLEMENT_OVERRIDE_FAILED",
                description: err?.message || "Protocol disbursement error.",
                variant: "destructive"
            });
        }
    });
    ...
    const handleExport = async () => {
        try {
            ...
            a.click();
            a.remove();
            toast({ title: "YIELD_REPORT_INITIALIZED", description: "Commission statement has been decrypted and downloaded." });
        } catch (error) {
            console.error("Export failed", error);
            toast({ title: "EXPORT_PROTOCOL_ERROR", description: "Failed to generate local yield statement.", variant: "destructive" });
        }
    };
    ...
    return (
        <div className="admin-commissions p-6 md:p-10 bg-[#F4F7FB] dark:bg-[#10221c] min-h-screen relative overflow-hidden">
            {/* Orbital UI Accents */}
            <div className="absolute top-[-10%] left-[-10%] opacity-[0.03] pointer-events-none">
                <Target size={800} className="animate-spin-slow text-primary rotate-45" />
            </div>
                    ...
                    <div>
                        <div className="flex items-center gap-4 mb-3">
                             ...
                             <h1 className="text-4xl md:text-5xl font-black uppercase italic tracking-tighter">
                                COMMISSION<span className="text-primary tracking-normal">_YIELD</span>
                             </h1>
                        </div>
                        <p className="text-xs font-black opacity-30 uppercase tracking-[6px] ml-7 italic">
                            Manage decentralized incentive architectures & platform profitability metrics
                        </p>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-8">
                         <div className="px-10 py-8 bg-white dark:bg-zinc-900/50 rounded-[3rem] border-2 border-orange-500/10 shadow-3xl flex flex-col items-center group hover:border-orange-500/40 transition-all backdrop-blur-xl relative overflow-hidden">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-[5px] opacity-30 mb-2 italic text-orange-500 flex items-center gap-3">
                                <Activity size={14} /> LIQUIDITY_PENDING
                            </span>
                         ...
                         <div className="px-10 py-8 bg-white dark:bg-zinc-900/50 rounded-[3rem] border-2 border-emerald-500/10 shadow-3xl flex flex-col items-center group hover:border-emerald-500/40 transition-all backdrop-blur-xl relative overflow-hidden">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-[5px] opacity-30 mb-2 italic text-emerald-500 flex items-center gap-3">
                                <CheckCircle size={14} /> AGENT_DISBURSEMENTS
                            </span>
            ...
            <div className="flex flex-col xl:flex-row items-center justify-between gap-10 mb-16">
                <div className="relative group w-full xl:w-[600px]">
                    ...
                    <input 
                        type="text" 
                        placeholder="LOCATE_YIELD_NODE_OR_AGENT..." 
                ...
                <div className="flex items-center gap-8 w-full xl:w-auto">
                    ...
                    >
                        <Download size={20} strokeWidth={4} className="text-primary group-hover:translate-y-1 transition-transform" /> EXPORT_LEDGER_DATA
                    </button>
                    <div className="hidden lg:flex items-center gap-6 bg-white dark:bg-zinc-900/50 px-8 py-4 rounded-3xl border border-border/50 backdrop-blur-md">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[5px] opacity-30 italic">REAL_TIME_YIELD_MONITORING: ON</span>
                    </div>
                </div>
            </div>

            {/* PAYOUT PROTOCOL MATRIX */}
            <div className="bg-white dark:bg-zinc-900/50 rounded-[4rem] border border-border/50 overflow-hidden shadow-2xl backdrop-blur-md">
                <div className="p-12 border-b border-border/50 flex flex-col md:flex-row md:items-center justify-between gap-10 bg-zinc-50 dark:bg-white/[0.02] relative">
                    ...
                    <div className="flex items-center gap-8 relative z-10">
                        ...
                        <div>
                            <h3 className="text-3xl font-black italic uppercase tracking-tighter leading-none">SETTLEMENT_QUEUE</h3>
                            <p className="text-[10px] font-black opacity-30 uppercase tracking-[5px] mt-3 italic ml-1 flex items-center gap-3">
                                <Satellite size={12} className="text-primary" /> Multi-agent commission sync protocol
                            </p>
                        </div>
                    </div>
                    <div className="flex items-center gap-4 bg-primary/5 text-primary px-8 py-4 rounded-[1.5rem] border border-primary/20 relative z-10 shadow-lg group">
                        ...
                        <span className="text-[11px] font-black uppercase tracking-[4px] italic">
                            {stats.pendingCount} PROTOCOLS_PENDING_DISBURSEMENT
                        </span>
                    </div>
                </div>
                    ...
                    <table className="w-full text-left font-black text-[10px] uppercase tracking-widest italic">
                        <thead>
                            <tr className="bg-zinc-100 dark:bg-white/5 text-[9px] opacity-40">
                                <th className="px-12 py-8 tracking-[4px]">FIELD_OPERATOR</th>
                                <th className="px-12 py-8 tracking-[4px]">CLIENT_DNA</th>
                                <th className="px-12 py-8 tracking-[4px]">PROTECTION_ARTIFACT</th>
                                <th className="px-12 py-8 text-center tracking-[4px]">YIELD_VALUATION</th>
                                ...
                                >
                                    ...
                                    <td className="px-12 py-8">
                                        <div className="flex flex-col gap-2">
                                            ...
                                            <span className="text-[8px] opacity-20 tracking-[3px] uppercase ml-4">VERIFIED_ENTITY</span>
                                        </div>
                                    </td>
                                    <td className="px-12 py-8">
                                    ...
                                    <td className="px-12 py-8">
                                        <div className="flex justify-center">
                                            <div className={`px-8 py-3 rounded-2xl text-[9px] font-black tracking-[4px] flex items-center gap-4 w-fit shadow-xl italic ${
                                                ...
                                                {comm.status === 'Paid' ? 'SETTLEMENT_COMMITTED' : 'AWAITING_ORCHESTRATION'}
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-12 py-8 text-right">
                                        {comm.status === 'Pending' ? (
                                            ...
                                            >
                                                INITIALIZE_PAYOUT <ArrowUpRight size={18} strokeWidth={4} className="group-hover/btn:translate-x-1 group-hover/btn:-translate-y-1 transition-transform" />
                                            </button>
                                        ) : (
                                            <div className="flex items-center justify-end gap-5 opacity-20 font-black italic text-[10px] uppercase tracking-[5px] group-hover:opacity-40 transition-opacity">
                                                <Zap size={18} /> PROTOCOL_FINALIZED
                                            </div>
                                        )}
                                    </td>
                                </motion.tr>
                            ...
                            {filteredComms?.length === 0 && (
                                <tr>
                                    <td colSpan={6} className="px-12 py-40 text-center">
                                        ...
                                        <h3 className="text-4xl font-black uppercase italic tracking-tighter opacity-10">NULL_YIELD_METRICS</h3>
                                        <p className="opacity-10 max-w-sm mx-auto mt-6 font-black uppercase text-[10px] tracking-[6px] italic leading-loose">No strategic data nodes identified in current ledger sweep.</p>
                                    </td>
                                </tr>
                            )}
                        </tbody>

...
import { 
    ...
    Fingerprint, Cpu, Satellite, BarChart3,
    IndianRupee, AlertCircle, Box, Layers,
    Command, Radar, Layout
} from "lucide-react";
...
const AdminDashboard = () => {
    ...
    const statsCards = [
        {
            ...
            description: "Live surveillance of all decentralized assets",
            icon: GlobeIcon,
        ...
        {
            title: "ENTITY_NETWORK",
            description: `${statsData?.stats?.totalCustomers || 0} Operators`,
            icon: Users,
            className: "col-span-1",
            background: <div className="absolute inset-0 bg-indigo-500/5 dark:bg-indigo-500/10" />
        },
        {
            title: "STRATEGIC_FORCE",
            description: `${statsData?.stats?.totalAgents || 0} Field Agents`,
            icon: Target,
            className: "col-span-1",
    ...
    return (
        <div className="admin-dashboard p-6 md:p-10 bg-[#F4F7FB] dark:bg-[#10221c] min-h-screen relative overflow-hidden">
                ...
                <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-10 mb-16">
                    <div>
                        <div className="flex items-center gap-4 mb-3">
                             ...
                             <h1 className="text-4xl md:text-5xl font-black uppercase italic tracking-tighter">
                                COMMAND<span className="text-primary tracking-normal">_CENTER</span>
                             </h1>
                        </div>
                        <p className="text-xs font-black opacity-30 uppercase tracking-[6px] ml-7 italic">
                            Universal system orchestration & high-fidelity asset monitoring
                        </p>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-6 bg-white dark:bg-zinc-900/50 backdrop-blur-md shadow-2xl border border-border/50 px-10 py-6 rounded-[2.5rem] relative">
                         <div className="absolute -top-3 -right-3">
                            <Satellite size={20} className="text-primary animate-bounce" />
                         </div>
                         <div className="flex flex-col items-center">
                            <span className="text-[9px] font-black uppercase tracking-[4px] opacity-20 mb-2 italic">ORBIT_SYNC</span>
                            <div className="flex items-center gap-3">
                                <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse shadow-[0_0_10px_#10b981]" />
                                <span className="text-xs font-black italic tracking-tighter uppercase">AUTHORIZED_ACTIVE</span>
                            </div>
                         </div>
                         <div className="w-px h-10 bg-border/20" />
                         <div className="flex flex-col items-center">
                            <span className="text-[9px] font-black uppercase tracking-[4px] opacity-20 mb-2 italic">SYSTEM_LATENCY</span>
                            <span className="text-xs font-black italic tracking-tighter uppercase text-primary">0.0042 MS</span>
                         </div>
                         <div className="w-px h-10 bg-border/20" />
                         <div className="flex flex-col items-center min-w-[120px]">
                            <span className="text-[9px] font-black uppercase tracking-[4px] opacity-20 mb-2 italic">UPTIME_INDEX</span>
                            <span className="text-xs font-black italic tracking-tighter uppercase text-accent">99.999%</span>
                         </div>
                    </div>
                </div>
            </Reveal>
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between mb-16 relative z-10 gap-8">
                            ...
                            <div className="flex flex-col items-end">
                                <div className="flex items-center gap-3 text-emerald-500 bg-emerald-500/10 px-6 py-3 rounded-2xl text-xs font-black border border-emerald-500/20 shadow-[0_0_20px_rgba(16,185,129,0.1)] italic">
                                    <ArrowUpRight className="w-5 h-5" strokeWidth={4} /> +12.8% YIELD_DELTA
                                </div>
                                <span className="text-[8px] font-black opacity-20 uppercase tracking-[3px] mt-2 mr-2">LAST_24_CYCLES</span>
                            </div>
                        </div>
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={performanceData}>
                                    ...
                                    <Tooltip 
                                        ...
                                        contentStyle={{ borderRadius: '32px', border: '1px solid rgba(1, 101, 255, 0.2)', boxShadow: '0 40px 80px -20px rgba(0,0,0,0.3)', fontWeight: 'black', background: 'rgba(16, 34, 28, 0.95)', textTransform: 'uppercase', fontSize: '10px', color: '#fff', backdropBlur: '10px' }}
                                    />
                    ...
                    <div className="bg-white dark:bg-zinc-900/50 p-12 rounded-[4rem] border border-border/50 shadow-2xl flex flex-col h-full relative overflow-hidden group backdrop-blur-md">
                        ...
                        <div className="mt-12 pt-12 border-t border-border/20 grid grid-cols-2 gap-10 relative z-10">
                             <div className="text-center group-hover:translate-x-2 transition-transform">
                                <p className="text-[9px] font-black opacity-30 uppercase tracking-[4px] mb-3 italic">SYST_EFFICIENCY</p>
                             ...
                             <div className="text-center group-hover:translate-x-[-2px] transition-transform">
                                <p className="text-[9px] font-black opacity-30 uppercase tracking-[4px] mb-3 italic">ORBITAL_INTEGRITY</p>
                ...
                <div className="bg-white dark:bg-zinc-900/50 rounded-[4.5rem] border border-border/50 shadow-[0_60px_100px_rgba(0,0,0,0.4)] overflow-hidden mb-16 backdrop-blur-xl">
                    <div className="px-12 py-12 border-b border-border/50 flex flex-col xl:flex-row xl:items-center justify-between gap-10 bg-zinc-50 dark:bg-white/[0.02] relative">
                        ...
                        <div className="flex items-center gap-6">
                            <div className="px-6 py-3 bg-white dark:bg-zinc-900/80 rounded-2xl border border-border/50 shadow-inner flex items-center gap-4">
                                ...
                                <span className="text-[9px] font-black uppercase tracking-[3px] opacity-40">FEED_SYNCED</span>
                            </div>
                    ...
                    <div className="p-10 space-y-6">
                        {statsData?.recentActivities?.length === 0 ? (
                            <div className="py-40 text-center relative">
                                ...
                                <h4 className="text-4xl font-black uppercase italic tracking-tighter opacity-10">NULL_SIGNALS_DETECTED</h4>
                                <p className="text-[10px] font-black uppercase tracking-[8px] opacity-10 mt-6 italic">Adjust orbital parameters to capture matching telemetry stream.</p>
                            </div>
                        ) : (
                                ...
                                >
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-6 opacity-30">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <p className="text-[10px] font-black uppercase tracking-[4px] italic">{new Date(activity.date).toLocaleTimeString().toUpperCase()} [GMT+5:30]</p>
                                                </div>
                                                <div className="w-2 h-2 bg-zinc-400/20 rounded-full" />
                                                <p className="text-[10px] font-black uppercase tracking-[4px] italic">SECTOR_NODE_04D</p>
                                                <div className="w-2 h-2 bg-zinc-400/20 rounded-full" />
                                                <span className="text-[10px] font-black text-emerald-500/50 uppercase tracking-[3px]">DATA_STABLE</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="mt-8 lg:mt-0 flex items-center gap-8 bg-zinc-100 dark:bg-white/5 p-4 rounded-2xl group-hover:bg-white dark:group-hover:bg-zinc-900 transition-colors">
                                         ...
                                         <button className="h-14 px-10 bg-white dark:bg-zinc-900 border-2 border-border/50 text-zinc-400 group-hover:text-primary group-hover:border-primary/50 group-hover:shadow-xl group-hover:shadow-primary/10 rounded-2xl font-black text-[10px] uppercase tracking-[4px] transition-all flex items-center gap-4 italic active:scale-95">
                                            TRACE_ENTITY <Zap size={16} strokeWidth={4} className="group-hover:scale-125 transition-transform" />
                                         </button>
                                    </div>
                                </motion.div>
                            ))
                    ...
                    <div className="p-12 bg-zinc-950 dark:bg-zinc-900 border-t border-white/5 flex flex-col xl:flex-row items-center justify-between gap-10">
                        <div className="flex items-center gap-6">
                            ...
                            <p className="text-xs font-black text-white/50 uppercase tracking-[6px] italic">Global Uplink: Established @ ORBIT_A1_77</p>
                        </div>
                        <div className="flex flex-wrap items-center gap-12 text-[10px] font-black uppercase tracking-[4px] text-white/30">
                            <span className="flex items-center gap-4 italic group cursor-pointer hover:text-primary transition-colors">
                                <Satellite size={18} className="text-primary group-hover:animate-spin-slow" /> ORBITAL_ALPHA_PRIME
                            </span>
                            <span className="flex items-center gap-4 italic group cursor-pointer hover:text-accent transition-colors">
                                <Satellite size={18} className="text-accent group-hover:animate-spin-slow" /> ORBITAL_BETA_NODE
                            </span>
                            <div className="w-px h-8 bg-white/10" />
                            <div className="flex items-center gap-4">
                                <Layout size={18} />
                                <span>v4.8.2_STABLE</span>
                            </div>
                        </div>
                    </div>
                </div>

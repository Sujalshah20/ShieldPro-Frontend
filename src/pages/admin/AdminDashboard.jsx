...
const AdminDashboard = () => {
    const stats = [
        { label: "Net_Premiums", value: "₹4.82M", trend: "+12.5%", icon: DollarSign, trendUp: true, tag: "FISCAL_NODE" },
                ...
                <Reveal direction="left">
                    <div className="space-y-4">
                        ...
                        <h1 className="text-5xl md:text-7xl font-black text-[#003249] uppercase tracking-tighter italic leading-none">
                            Mainframe <span className="text-[#007ea7]">Overview_</span>
                        </h1>
                        <p className="max-w-xl text-slate-400 font-bold uppercase tracking-widest text-xs italic leading-relaxed">
                            Centralized infrastructure monitoring and security protocol management. Current Grid Health: <span className="text-emerald-500">OPTIMIZED</span>
                        </p>
                    </div>
                </Reveal>
                        ...
                        <div className="saas-card group relative p-10 border-2 border-slate-50 hover:border-[#007ea7]/30 transition-all duration-700 min-h-[280px] flex flex-col justify-between overflow-hidden">
                            ...
                            <div className="relative z-10">
                                <p className="text-[10px] font-black text-slate-300 uppercase tracking-[6px] mb-4 italic flex items-center gap-3">
                                    <Briefcase size={12} className="text-[#007ea7]" /> {s.label}
                                </p>
                            ...
                            <table className="w-full text-left">
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {activePolicies.map((p, idx) => (
                                        <tr key={p.id} className="hover:bg-slate-50/50 transition-all duration-500 group cursor-pointer relative">
                                            ...
                                            <td className="px-12 py-10">
                                                <div className="flex items-center gap-6">
                                                    ...
                                                    <div className="flex flex-col">
                                                        ...
                                                        <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest italic opacity-40">VERIFIED_HOLDER_0{idx + 1}</span>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-12 py-10">
                                            ...
                                            <td className="px-12 py-10 text-center">
                                                <span className={`px-5 py-2 rounded-xl text-[9px] font-black uppercase tracking-[4px] border-2 shadow-lg italic ${
                                                    ...
                                                    p.status === 'Pending' ? 'text-amber-500 bg-amber-50 border-amber-50' : 'text-rose-500 bg-rose-50 border-rose-50'
                                                }`}>
                                                    {p.status}
                                                </span>
                                            </td>
                                            ...
                                            <td className="px-12 py-10 text-right">
                                                <button className="w-12 h-12 flex items-center justify-center text-slate-300 hover:text-[#007ea7] hover:bg-white rounded-xl transition-all shadow-sm">
                        ...
                        <div className="saas-card relative overflow-hidden group p-12 shadow-3xl border-2 border-slate-50">
                            ...
                            <div className="space-y-10 relative z-10">
                                {[
                                    ...
                                    <div key={a} className="group/stat">
                                        <div className="flex justify-between items-end mb-4">
                                            <div className="flex items-center gap-3">
                                                <span className="text-[10px] font-black uppercase tracking-[5px] text-slate-400 italic leading-none group-hover/stat:text-[#003249] transition-colors">{t.label}</span>
                                            </div>
                                            <span className="text-xl font-black text-[#003249] italic tracking-tighter leading-none">{t.val}%</span>
                                        </div>
                                        <div className="h-4 bg-slate-50 border-2 border-slate-50 rounded-full overflow-hidden shadow-inner p-0.5">
                                            <motion.div 
                                                initial={{ width: 0 }}
                                                animate={{ width: `${t.val}%` }}
                                                transition={{ duration: 2, ease: "circOut", delay: a * 0.2 }}
                                                className={`h-full ${t.color} rounded-full shadow-[0_0_15px_rgba(0,126,167,0.4)] relative`}
                                            >
                        ...
                        <div className="saas-card relative overflow-hidden p-12 shadow-3xl border-2 border-slate-50 flex flex-col min-h-[460px]">
                            ...
                            <div className="space-y-10 relative z-10 flex-1">
                                {[
                                    ...
                                    <div key={a} className="flex gap-8 group">
                                        <div className="flex flex-col items-center gap-3">
                                            <div className="w-3 h-3 rounded-full shadow-[0_0_15px_currentcolor] group-hover:scale-150 transition-transform" style={{ color: t.color, backgroundColor: t.color }} />
                                            <div className="w-0.5 flex-1 bg-slate-50 group-last:bg-transparent" />
                                        </div>
                                        <div className="flex-1 pb-10 group-last:pb-0">
                                            <div className="flex justify-between items-center mb-2">
                                                <p className="text-sm font-black text-[#003249] uppercase tracking-tighter italic group-hover:text-[#007ea7] transition-colors">{t.title}</p>
                                                <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest italic">{t.time}</span>
                                            </div>
                                            <p className="text-[11px] font-black text-slate-400 uppercase tracking-[2px] leading-relaxed italic mb-3 opacity-60">{t.detail}</p>
                                            <span className="px-3 py-1 bg-slate-50 border border-slate-100 rounded-lg text-[7px] font-black text-slate-400 uppercase tracking-widest italic">{t.tag}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
